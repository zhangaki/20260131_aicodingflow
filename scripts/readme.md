...
### `01-01_topic_researcher.py` (The Editor-in-Chief)
...
python scripts/01-01_topic_researcher.py

# Find topics for a specific niche
python scripts/01-01_topic_researcher.py --niche "Python Web Frameworks" --count 3
```
...
### `02-01_unified_content_workflow.py` (The Flagship)
...
python scripts/02-01_unified_content_workflow.py --topic "Future of AI Agents" --mode general

# News Brief (Short, punchy, trend-focused)
python scripts/02-01_unified_content_workflow.py --topic "OpenAI o3 Release" --mode news

# Detailed Guide (Step-by-step, tutorial style)
python scripts/02-01_unified_content_workflow.py --topic "How to fine-tune Llama 3" --mode guide
```
...
### `02-02_programmatic_seo_engine.py` (The Bulk Generator)
...
python scripts/02-02_programmatic_seo_engine.py --type comparison

# Generate listicle articles (e.g., Top 10 Tools)
python scripts/02-02_programmatic_seo_engine.py --type listicle
```

### `02-03_batch_generate_deep_dives.sh` (The Batch Runner)
**Purpose:** Shell script to execute multiple `02_unified_content_workflow.py` commands in sequence.
**Usage:**
```bash
# Run the batch generation
./scripts/02-03_batch_generate_deep_dives.sh
```
...
### `03-01_content_expander.py` (The Scaler)
...
python scripts/03-01_content_expander.py --mode thin

# Expand specific article by slug
python scripts/03-01_content_expander.py --slug "my-short-post"
```

### `03-02_link_builder.py` (The Connector)
**Purpose:** Internal linking and graph structure.
**Usage:**
```bash
# Build internal links based on semantic relevance
python scripts/03-02_link_builder.py --build

# Audit for broken internal links
python scripts/03-02_link_builder.py --audit
```
...
### `04-01_image_manager.py` (The Visual Engine)
...
python scripts/04-01_image_manager.py --action generate --prompt "A futuristic cyberpunk city, neon lights, 8k" --output "assets/cyber-city.png"

# 2. Optimize ALL assets (Convert to WebP, Resize >1200px)
python scripts/04-01_image_manager.py --action optimize

# 3. Audit for missing images or unused "zombie" files
python scripts/04-01_image_manager.py --action audit
```

### `04-02_blog_maintenance.py` (The Janitor)
**Purpose:** Keeps the codebase clean, consistent, and error-free.
**Usage:**
```bash
# Fix Markdown structuring (Tables, Code Fences, Trapped Text)
python scripts/04-02_blog_maintenance.py --action format

# Audit for structural issues (Read-only check)
python scripts/04-02_blog_maintenance.py --action audit

# Standardize Frontmatter (Dates, Tags, Descriptions)
python scripts/04-02_blog_maintenance.py --action clean

# Add 'noindex' to thin content
python scripts/04-02_blog_maintenance.py --action noindex --min-words 500
```

### `04-03_content_quality_scan.py` (The Deep Auditor)
**Purpose:** Deep-scans for common AI patterns, fake statistics, and outdated technical references.
**Usage:**
```bash
# Run a full scan and output a risk report
python scripts/04-03_content_quality_scan.py

# Export results to CSV for batch processing
python scripts/04-03_content_quality_scan.py --export
```
...
**Usage:**
```bash
node scripts/05-01_generate_llm_bundle.js
```

---

## ⚠️ Workflow Rules
1. **Always Optimize Images:** Run `scripts/04-01_image_manager.py --action optimize` after adding any new image.
2. **Check Structure:** Run `scripts/04-02_blog_maintenance.py --action audit` before pushing.
